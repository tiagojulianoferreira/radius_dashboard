...
def parse_log_file(filepath, stats, today_only=False):
    ...
    with f:
        for line in f:
            ...
            # Se for only today, pula logs que não são de hoje
            ...
            if fail_match:
                ...
                if sess_id not in processed_sessions:
                    ...
                    if current_date:
                        ...
                        # Série histórica (agrupada por dia)
...
def parse_all_logs(log_pattern="/var/log/freeradius/radius.log*", today_only=False):
    """Analisa todos os arquivos de log (incluindo compactados)"""
    ...
    # Converte histórico para lista ordenada
...
def generate_html(stats, stats_today, chart_js_code):
    """Gera o HTML do dashboard com todas as análises"""
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .badge {{ background: #6c757d; color: white; padding: 5px 10px; border-radius: 4px; font-size: 0.85em; }}
        ...
        .today-stats .stat-item .number {{
            font-size: 32px;
            font-weight: 700;
        }}
        .today-stats .stat-item .label {{
            font-size: 12px;
            opacity: 0.8;
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <p style="font-size: 11px; color: #999; margin-top: 5px;">Acessível via: https://seu-dominio/radius/</p>
        </div>
    </div>
...
def main():
    print("📊 Gerando Dashboard RADIUS com análise histórica...")
    ...
    print(f"   👤 Usuários com falhas: {len(stats['users_fail'])}")
    ...
    total = stats['success'] + stats['fail']
    if total > 0:
        print(f"   📊 Taxa de falhas histórica: {stats['fail']/total*100:.2f}%")
    
    print("\n📊 Estatísticas do Dia Atual:")
    print(f"   ✅ Sucessos hoje: {stats_today['success']:,}")
    print(f"   ❌ Falhas hoje: {stats_today['fail']:,}")
    total_today = stats_today['success'] + stats_today['fail']
    if total_today > 0:
        print(f"   📊 Taxa de falhas hoje: {stats_today['fail']/total_today*100:.2f}%")
